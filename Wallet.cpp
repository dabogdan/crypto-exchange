...
bool Wallet::canFulfilOrder(const OrderBookEntry& order) {
    std::vector<std::string> currs = CSVReader::tokenise(order.product, '/');
    if (order.orderBookType == OrderBookType::ask) {
        double amount = order.amount;
        std::string currency = currs[0];
        std::cout << "Wallet::canFulfilOrder " << currency << " : " << amount << std::endl;
...
void Wallet::processSale(OrderBookEntry& sale){
    ...
    if (sale.orderBookType == OrderBookType::asksale) {
        ...
        std::cout << "Wallet::processSale " << outgoingCurrency << " : " << outgoingAmount << std::endl;
    ...
    if (sale.orderBookType == OrderBookType::bidsale) {
        ...
        std::string imcomingCurrency = currs[0];
        double outgoingAmount = sale.amount * sale.price;
        std::string outgoingCurrency = currs[1];
        std::cout << "Wallet::processSale " << outgoingCurrency << " : " << outgoingAmount << std::endl;
        currencies[imcomingCurrency] += incomingAmount;
        currencies[outgoingCurrency] -= outgoingAmount;
    }
}
